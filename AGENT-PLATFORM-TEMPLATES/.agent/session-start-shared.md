...
- Do not narrate that you are reading files — but **always** output the `▶` routing status line (as defined in AGENTS.md Section 2) as the first line of your response when routing fires; this is the platform's visible signal to the user and is NOT suppressed by caveman mode, compression mode, or any brevity instruction
