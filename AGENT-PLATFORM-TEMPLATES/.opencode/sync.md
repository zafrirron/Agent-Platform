...
Framework: **opencode**. Protocol: `.agent/SYNC.md`

Session start: `Read .opencode/prompts/session-start.md and execute it.`
Session end:   `Read .opencode/prompts/session-end.md and execute it.`

Do not edit `.cursor/`, `.claude/`, `.agents/`, `.codex/`.

## Session awareness (check once per chat)

On your **first response** of this chat:
1. Read `.agent/handoff/sync/registry.yaml`
2. If `frameworks.opencode.status` is `idle` — output this notice on one line before your response:
