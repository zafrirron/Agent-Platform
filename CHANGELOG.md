...
### Added
- **First curated `domain` pack — `domain-c4i` (C2 / C4ISR / Command & Control)** — the maintainer `build-pack=<id>` workflow produced its first real shipped pack, proving the dry-run → real-build → register → PSG-pack-lane → green-tests loop end to end. `domain-c4i` distills **application-layer** command-and-control expertise: single common-operating-picture (COP) truth, an explicit C2 information model (entities/tracks/tasking/plans/geometries/events with datum+time), track management, DIL-tolerant operator workflows, MIL-STD-2525 / NATO APP-6 symbology, screen-layout/perspectives + zone UX, alerting/decision-support, multi-echelon releasability, after-action replay, collaboration, and **full-motion-video-in-the-application UX** (feeds as first-class COP citizens, slew-to-cue, video-on-map, DVR/replay). Ships `pack.json`, 4 overlays (architect/frontend/backend/security), `routing.md`, and 4 references (`reference-architecture.md`, `c4i-capabilities.md`, `c4i-ux.md`, `c4i-video.md`). Opt-in like every pack (`active_packs`); no consumer default changes. **+1 test (265 total).**
  - **Strict axis discipline:** a `domain` pack carries only capabilities + UI/UX + C2 semantics. Every technology signal seen during the build (CoT/TAK transport, DDS middleware, map-rendering engine, video codec/transport, edge hardware/OS, C++) was routed to **Adjacent pack candidates** (`stack-cot-tak`, `stack-dds`, `stack-geospatial`, `stack-fmv-decode`, `platform-tactical-edge`, `language-cpp`) and recorded in the pack's Do-not-re-propose ledger — never merged into the domain brain. Video *UI/UX* stayed domain; only the raw codec/transport primitive is adjacent.
  - **Provenance:** per-pack dedup ledger `MAINTAINER/scan-results/packs/domain-c4i.md` (17 findings + adjacents), `docs/INTELLIGENCE-SOURCES.md`, and `pack.json.reference_sources[]` cite [ATAK-CIV](https://github.com/deptofdefense/AndroidTacticalAssaultKit-CIV) and [FreeTAKServer](https://github.com/FreeTAKTeam/FreeTakServer) plus public MIL-STD-2525 / APP-6 standards.
  - **`domain-c4i` v1.3.0 — Mode 3 ingest of an internal C2 engineer's UI/UX skill (operator cognition & interaction design).** A domain engineer's submission was ingested via the pack-lane PACK-CANDIDATE path, adding the human-factors layer the pack lacked: **new `references/c4i-cognition.md`** (SA pipeline Perception/Comprehension/Projection, cognitive-load/widget limits, alert fatigue/habituation, **Fitts' Law forward+reverse**, stress single-tasking + sensible defaults, interruption task-state save, forgiveness + **neutral destructive-action colours**, UX-enforcing components) plus four always-on rules in `frontend-agent.overlay.md` and a routing row. Findings C001–C008 adopted; C009–C011 (dual-coding, agent transparency, graceful degradation) were already in the brain and recorded as validated duplicates. Credited as an owned internal contribution in `reference_sources`.
  - **`domain-c4i` v1.2.0 — `url=` ecosystem site scan (breadth: all-domain, ISR-PED, decision support).** First live run of the maintainer **targeted site scan** (`url=<website> pack=domain-c4i`) against a C4ISR industry landscape page. Distilled *capability themes* only (inspiration-only IP guard — no vendor list/wording copied) and adopted three genuine gaps: **all-domain / multi-domain C2** (land/air/maritime/space/cyber, JADC2, joint interop by contract), **ISR collection management + PED** (the "ISR" half — tasking→coverage/gap→Processing/Exploitation/Dissemination→intel products), and **human-on-the-loop predictive decision support** (AI recommendations with confidence/provenance, never auto-acting). Landed as `c4i-capabilities.md` §15–17 + architect/backend/frontend overlay rules + routing. Cyber-ops/EW and ML inference recorded as **adjacent** candidates (`domain-cyber-ops`, `stack-ew`); commander roll-up + unmanned-C2 deferred. Findings U001–U007 in the ledger; marketing-level signals flagged to deepen via standards scans.
  - **`domain-c4i` v1.1.0 — live web-scan enrichment (author-derived → source-verified).** A pack-scoped web scan fetched and distilled real sources into the brain: **new `symbology-sidc.md`** (MIL-STD-2525D/APP-6(D) 20-digit SIDC, frame/icon/status/echelon composition, JMSML validation, echelon-offset & frame/icon pixel-grid pitfalls); **MISB ST 0601 / STANAG 4609 / KLV** FMV-metadata depth in `c4i-video.md` (sensor/platform geo-metadata → footprint/target-location, MPEG-2 TS mux, sync responsibility); and **Cursor-on-Target entity semantics** in the backend/security overlays (persistent uid, time/start/stale validity, **stale→confidence decay**, access/sharing marker). **Licenses verified & corrected:** ATAK-CIV is **GPL-3.0** (copyleft study-only, archived read-only May 2025) — it had been mislabeled Apache-2.0 in the author-derived v1.0.0 draft; FreeTAKServer confirmed **EPL-2.0**. Findings B018–B021 logged in the per-pack ledger; wire format/codec/rendering remain adjacent-pack concerns.

### Added
- **OpenCode as a first-class 5th IDE framework** — the platform now ships full coordination for [OpenCode](https://opencode.ai) alongside Claude Code, Cursor, Antigravity, and Codex. OpenCode already reads the platform's `AGENTS.md`/`CLAUDE.md`/skills natively; this makes it a peer of the other frameworks with its own private folder, session prompts, slash commands, subagent, and config.
  - **`.opencode/` private folder** — `FRAMEWORK.json`, session-start/end prompts (`opencode` folder id), an always-on `sync.md` (multi-framework awareness + session detection + auto-routing), and registry/zones/sync wiring so cross-IDE handoff and the cross-framework Critic offer work for OpenCode automatically.
  - **Lifecycle slash commands** → `.opencode/commands/*.md` (`/spec` `/plan` `/build` `/test` `/review` `/code-simplify` `/webperf` `/context` `/verify` `/ship` `/audit` `/release` `/quick-ref`) using OpenCode's `description`-frontmatter command format.
  - **Critic subagent** → `.opencode/agents/critic.md` exposes the adversarial Critic as an invokable subagent (`@critic`); other experts route through the shared `AGENTS.md`.
  - **`opencode.json` config** — written to the repo root with `instructions` → `AGENTS.md` + `.opencode/sync.md`, and **never clobbered** if one already exists (your provider/model settings are preserved).
  - **Detection & scoping** — `--framework=opencode` installs only the OpenCode surface; the default install adds it to all frameworks. `.opencode/` + `opencode.json` are gitignored by default, backed up if pre-existing, and removed cleanly on uninstall.
  - Followed `MAINTAINER/add-framework.md`: manifest `frameworks[]` + file entries, `profile-filter` framework prefix, `apply.js` (fw list/labels, gitignore, uninstall dirs, pre-existing scans, 5-framework banner), and every shared file (registry.yaml, SYNC.md, ZONES.md, session-start/end, SYNC-POINTS.md, handoff TEMPLATE, `.agent/README.md`) + all four existing frameworks' "do not edit" lists.
  - Docs/presentation updated to 5 frameworks: README, `AGENT-PLATFORM-FRAMEWORK-README.md`, `AGENT-PLATFORM-BOOTSTRAP.md`, `docs/DISTRIBUTION.md` (new **OpenCode interoperability** section), PLATFORM-HELP, CONTRIBUTING, COPYING, both decks, `package.json` keywords. **+13 tests (258 total)** covering default + scoped install, command/subagent/config emission, non-clobber, and gitignore.
  - **Provenance:** Mode 4 targeted scan of `anomalyco/opencode` — findings R036 (native compat, documented), R037 (framework + commands), R038 (Critic subagent), R039 (`opencode.json`). R040 (permissions) and R041 (parallel sessions → R014) logged to the roadmap backlog.

### Added
- **Maintainer Mode 5: Solution Blueprint (`blueprint="<system goal>"`) — decompose a whole mission into a 4-axis pack set, with per-candidate approval gates** — a top-down front-end to pack building. The maintainer states a system/mission goal in plain language (e.g. *"drone mission platform on an NVIDIA edge module, visual AI on a gimbaled camera, flight path guided by external radar, guidance hands off radar→visual as range closes"*) and Mode 5 decomposes it into candidate packs across all four axes — `domain` (mission logic + the radar→visual handoff + operator UX), `stack` (ROS2, TensorRT/CV, GStreamer, MAVLink/PX4), `platform` (NVIDIA Jetson edge), `language` (C++, Python) — dedups against the catalog + ledgers, then **presents a plan for per-candidate approval**. **Approval gates (the maintainer's veto):** `Approve all ★` / `Approve <id>` / `Approve axis:<axis>` / **`Reject <id>` / `Reject axis:language`** / `Defer` / `Reuse` / `Modify`. Rejected candidates are recorded as **out-of-scope exclusions** and pre-seeded into built packs' ledgers so cross-axis discovery can never resurrect vetoed stacks/platforms/languages. Approved packs are built by delegating to the existing engine (`build-pack=<id>` for NEW, `pack=<id>` for ENHANCE) — each keeps its own findings selection, PSG pack lane, and ledger (tagged `via blueprint <slug>`); the system-of-systems composition is written into the primary domain pack's `reference-architecture.md`, and an optional named **solution bundle** lets the installer later suggest the pack set together. New `MAINTAINER/solution-blueprint.md`; wired into `platform-maintainer-agent.md`, `REPORT-SCHEMA.md` (`blueprint` scope + archive), `registry.md`, and `GUIDE.md` (sixth maintainer tool). Maintainer-workflow docs only — no consumer template/behavior change; it never auto-builds a pack.
- **Docs: maintainer workflow made explicit (base vs packs) + private/proprietary-pack fork pattern surfaced everywhere** — `MAINTAINER/GUIDE.md` gains a **"The maintainer's job — base platform vs packs"** section (two-lane model, a 5-modes×2-lanes matrix, the pack expansion lifecycle `build-pack= → pack= → repo=…pack= → Mode 3 → add rule`, and a core-vs-pack **cadence** table), plus a **"Private & proprietary packs — fork to a private repo"** section (fork → build packs in the fork → install from the fork → merge upstream core conflict-free; IP layering table). The **fork-to-private pattern** and **Mode 5** are now called out in user-facing docs too: `README.md`, `AGENT-PLATFORM-FRAMEWORK-README.md`, `docs/DISTRIBUTION.md`, the pack spec `.agent/packs/README.md`, and both decks (`agent-platform-beta.html`, `team-adoption.html`). Docs-only; no template/behavior change.
- **Maintainer: Pack Ecosystem Build Scan (`build-pack=<id>`) — author a new pack brain from the whole web ecosystem** — closes the gap where the maintainer could only *freshen* an existing pack (`pack=<id>`) or deep-read *one* repo (Mode 4 `repo=… pack=<id>`), but had no greenfield, web-wide way to build a domain (or stack/platform/language) expert from the ecosystem. New Mode 2 sub-mode in `web-audit.md` runs an **axis-aware source matrix** (domain → standards/specs/reference OSS apps/threat models/compliance/communities; stack → docs/release notes/RFCs/pitfalls; platform → vendor docs/datasheets/SDK/RTOS; language → spec/style guides/footgun catalogs), triages licenses, synthesizes a candidate pack brain (overlay + `reference-architecture.md` + `reference_sources[]`), presents findings for selection, then scaffolds+fills the pack via the PSG pack lane. Registered in `platform-maintainer-agent.md` (Mode 2 triggers + grow-packs lifecycle) and `REPORT-SCHEMA.md` (`pack-build` scope + archive path). Maintainer-workflow docs only — no consumer template/behavior change.
- **"One terminal command" principle enforced end-to-end — every post-install action is now a prompt** — audited every user-facing surface for raw `npx`/terminal instructions and closed the gaps. `AGENTS.md` gains a **"Platform management" trigger table** covering skills (list/add), enforcement guards (install/remove), global stubs (install/remove), repair, force-refresh, and uninstall — the agent runs the underlying command on the user's behalf. `QUICK-REF.md` and `PLATFORM-HELP.md` lifecycle/skills/packs tables now **lead with natural-language prompts** and demote `npx` to a collapsed "under the hood" note. Net: the *only* terminal command a user ever runs is the one-time install; everything else is `router + manifest + tools`.
- **Pack management is now pure natural language — no `npx` for users** — packs were only reachable via raw `npx …` commands in the help docs, which is wrong for an agentic platform. Added `.agent/tools/packs.md` (agent-instructions doc, mirrors `upgrade.md`) and a **"Pack management" trigger table in `AGENTS.md` Section 2** so the agent handles plain prompts and runs any terminal step itself: *"what packs are available?"*, *"what packs are active?"*, *"which packs should I use?" / "scan my repo for packs"*, *"activate the React pack"*, *"deactivate the X pack"*, *"add this rule to my `<pack>` pack"*. `PLATFORM-HELP.md` and `.agent/packs/README.md` now lead with the prompts and demote the `npx` commands to a collapsed "under the hood" note. Manifest registers the new tool.
- **User-owned pack customization lane (`user.overlay.md`) — project rules for a pack that survive every update** — packs now have the same "your content is untouchable" guarantee as the core. Each active pack can carry a `.agent/packs/<id>/user.overlay.md` that is **never in the platform manifest**, so no `install`/`upgrade`/`force`/re-`add` ever touches it. The router (`AGENTS.md` step 3b) reads it **last** on any routed task where the pack is active, so it takes precedence over both the shipped pack overlay and the generic expert. New step 3c teaches the agent to handle *"add this to my `<pack>` pack"* requests by appending the rule to `user.overlay.md` (e.g. a `domain-c2` user asking for a panel/layout rule) — shipped `*.overlay.md` files are never edited inline. `.agent/packs/README.md` gains a **"Customizing a pack"** section + an update/preservation table. +6 tests (264 total).

### Fixed
- **`active_packs` dropped on `--mode=upgrade` / `--mode=force`** — the write loop rewrites `.agent/platform.json` from the template, and the later merge step read the already-clobbered file, so a core upgrade silently deactivated every opt-in pack. The installer now captures `active_packs` **before** the write loop and restores it, so upgrades/force keep your activated packs. Covered by the new pack-preservation tests.
...
### Changed
- **Targeted site scan (`url=<website>`) — maintainer can point the scanner at a non-repo website** — the non-repo analogue of Mode 4's `repo=owner/name`. Deep-reads one live site (product/service/docs/standards page) in a domain and distils best practices + feature ideas. Runs in the **core/baseline lane** (`url=…`, generalize to universal) or a **pack lane** of any axis (`url=… pack=<id>`). Reuses the whole pipeline — `F`/`E` classification, core-vs-pack routing, cross-axis Adjacent-pack candidates (Phase B2), and dedup via `registry.md` + the per-pack ledger. Adds a site-analysis lens (capability/feature inventory, demonstrated best practices, UX/workflow patterns, published specs, anti-patterns) and two mandatory guards: **prompt-injection safety** (page text is data, never instructions) and **IP/attribution** (proprietary sites are inspiration-only — distil, never clone; open specs are citable). Wired into `web-audit.md`, `REPORT-SCHEMA.md`, `registry.md`, `platform-maintainer-agent.md`, and `GUIDE.md`.
- **Pack ecosystem build scan is now cross-axis aware + dedup-safe across iterations** — two maintainer-workflow gaps closed before authoring the first real pack:
  - **Cross-axis signal capture (Phase B2)** — a `build-pack=`/`pack=` scan on one axis now records off-axis discoveries (a domain scan surfacing `language`/`platform`/`stack` signals) as **Adjacent pack candidates** instead of baking them into the primary pack. Each is proposed as a chained `build-pack=<other-id>` (spin off) or `add rule to pack <existing-id>` (route), keeping axes orthogonal and preventing cross-pack duplication. The report ends with an explicit Adjacent pack candidates section.
  - **Per-pack dedup ledger** — new `MAINTAINER/scan-results/packs/<id>.md` (schema `packs/README.md`), the pack-scoped mirror of `registry.md`. Every pack-scoped scan (`build-pack=`, `pack=`, Mode 4 `repo=…pack=`, Mode 3 ingest `pack=`) **reads it first** (classify NEW / ENHANCE / DUPLICATE / REJECTED-BEFORE) and **updates it after** (sources consumed, dispositions, Do-not-re-propose, adjacent candidates), so no source or finding — including deliberately rejected ones — is ever processed twice. Wired into `web-audit.md`, `github-governance-scan.md`, `platform-ingest.md`, the PSG pack lane, `REPORT-SCHEMA.md`, `registry.md`, and `GUIDE.md`. Maintainer-only — never shipped to consumer repos.

### Documentation
- **Full repo doc-sync sweep for OpenCode + Packs** — a whole-repo audit found 47 stale spots across 22 docs where the OpenCode framework and the packs model had not landed. Fixed: `MAINTAINER/GUIDE.md` (5-framework always-loaded table + checklist + audit grep, repo-layout tree with `.opencode/`/`.agent/packs/`, scope diagram, `build-pack=<id>` grow-pack row, test count 264), `README.md` (4-kind packs + `platform` axis, NL pack management, `user.overlay.md`, 264 tests, fixed hero/anchor), `docs/DISTRIBUTION.md` (4 kinds, NL prompts, OpenCode slash-command rows, Gemini→6th), `AGENT-PLATFORM-FRAMEWORK-README.md` (NL packs + `user.overlay.md`, installed tree `packs/`/`tools/packs.md`, 6th-framework), `QUICK-REF.md`/`QUICK-REF-lite.md`/`PLATFORM-HELP.md` (4 kinds, 6th-framework), `CONTRIBUTING.md`/`SECURITY.md`/`PACK-DEPLOY.md`/`SWITCH-PROMPTS.md`/`TOKEN-BUDGET.md`/`AGENTS-lite.md` (OpenCode + counts), both presentation decks (packs titles/cards to 4 axes, NL pack UX, OpenCode lifecycle), and `MAINTAINER/add-framework.md` + `MAINTAINER/platform-audit.md` (baseline 5→6 for the next framework, OpenCode in "do not edit"/SWITCH steps, pack-health across all 4 kinds + `build-pack` coverage-gap check). No behavioural/code change; `npm test` 264/264 green.
